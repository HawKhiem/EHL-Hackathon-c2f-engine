...
if [ "$USE_DB" -eq 1 ]; then
  ...
  if supabase status >/dev/null 2>&1; then
    ...
    if ! supabase start; then
      ...
      warn "Supabase project owns this branch's ports (54421-54427). Either:"
      ...
      printf "\n"
      docker ps --filter "publish=54422" --format '  holding 54322: {{.Names}}' 2>/dev/null || true
  ...
  [ -n "$API_URL" ] || API_URL="http://127.0.0.1:54421"
...
printf "  %ssupabase studio%s  http://127.0.0.1:54423\n" "$GREEN" "$OFF"
